...
function parseFieldErrors(
  ...
  const apiMessage = topLevelMessages.filter(Boolean).join('. ') || ''
  return { fieldErrors, apiMessage }
}

async function extractApiError(err: unknown): Promise<{ fieldErrors: Record<string, any>; apiMessage: string }> {
...
export function HotelPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { fieldErrors, apiMessage } = await extractApiError(err)
